---
name: tuicr
description: Use tuicr's review CLI to read and add comments in active TUI review sessions, and launch tuicr in Herdr when a user needs an interactive review pane.
---
...
Herdr just to connect to an existing active session.
...
| Not set | Tell the user you are waiting for them to start `tuicr` in the repo, then attach with `tuicr review list` after they say it is ready |
...
`tuicr review comments`; it is the primary source of review feedback.
...
| No active session, Herdr available | Start a new tuicr pane with the Herdr wrapper |
| No active session, Herdr not available | Tell the user you are waiting for them to start `tuicr` |
